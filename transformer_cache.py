...
def decoding_ctx(context, decoder_output, params):
    #############################################################
    # cache
...
def decoding_graph_catch(features, state, mode, params):
    ...
    if mode != "infer":
        ...
        ctx_ = decoding_ctx(context, decoder_output, params)
        decoder_output = decoder_output + 0*ctx_
        ############################################
...
def context_encoder(features, mode, params):
    ...
    encoder_context_output = encoding_graph(context_feature, mode, params)
    ...
    encode_heads = layers.attention.split_heads(encoder_context_output, params.num_heads)  # N*2(history) Heads L 64
    encode_heads = tf.transpose(encode_heads, [0, 2, 1, 3])  # N*2(history) L Heads 64
...
def model_graph(features, mode, params):
    ...
    # last_context = tf.reshape(context, [1, context_shape[0], context_shape[1]])
    # ##############################

    #last_context = tf.Print(last_context, [tf.shape(context), tf.shape(last_context)], 'tf.shape(context), tf.shape(last_context)', 100, 1000)
    #
    #####################################
    #encoder_output = tf.Print(encoder_output, [tf.shape(context)], 'tf.shape(context)', 100, 1000)
...
class Transformer(interface.NMTModel):
    ...
    def get_training_func(self, initializer):
        def training_fn(features, params=None, reuse=tf.AUTO_REUSE):
            if params is None:
