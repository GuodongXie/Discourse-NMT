...
import dataset_cache as dataset
    ...
    for name in features:
        ...
        if batch < num_shards:
            ...
            for i in range(num_shards):
                ...
                n = num_shards
    new_predictions = (predictions[0][:n], predictions[1][:n])  #这样确保留下有效的GPU OP没有错误
...
def main(args):
    ...
    model_cls_list = [transformer_cache.Transformer for model in args.models]
    ...
    with tf.Graph().as_default():
        ...
        # Read input file
        sorted_keys, sorted_inputs = dataset.sort_input_file_catch(args.input)
        # Build input queue
        ...
        for i in range(len(params.device_list)):
            placeholders.append({
                ...
                "source_length": tf.placeholder(tf.int32, [None],
                                                "source_length_%d" % i),
                "context": tf.placeholder(tf.int32, [None, None, None],
                                                "context_%d" % i),
                "context_sen_len": tf.placeholder(tf.int32, [None, None],
                                          "context_sen_len_%d" % i),
                "context_ori": tf.placeholder(tf.string, [None, None, None],
                                                  "context_ori_%d" % i),
                "source_ori": tf.placeholder(tf.string, [None, None],
                                         "source_ori_%d" % i),
